...
An equivalence from an opposite category turns a minimal right almost-split
morphism into a minimal left almost-split morphism.  The small generic proof is
adapted from the adjacent OP-conjecture formalization; no OP-specific theorem
...
/-- A left almost-split morphism becomes right almost split after passage to
the opposite category. -/
theorem IsLeftAlmostSplit.op
    {X Y : C} {f : X ⟶ Y} (hf : IsLeftAlmostSplit f) :
    IsRightAlmostSplit f.op := by
  constructor
  · intro hs
  ...
  · intro W g hg
    have hgnonsplit : ¬ IsSplitMono g.unop := by
      intro hsplit
      apply hg
    ...
    obtain ⟨h, hh⟩ := hf.factors g.unop hgnonsplit
    refine ⟨h.op, ?_⟩
    apply Quiver.Hom.unop_inj
    simpa only [unop_comp, Quiver.Hom.unop_op] using hh
...
/-- A right-minimal morphism becomes left minimal under an
anti-equivalence. -/
